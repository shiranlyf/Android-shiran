...
/**
 * 在开发应用时都会和Activity打交道，而Application使用的就相对较少 Application是用来管理应用程序的全局状态的，比如载入资源文件
...
public class LocalApplication extends BaseApplication {
	...
	public void onCreate() {
		...
		// 初始化数据库
		// dbUtils =
		// DbUtils.create(LocalDaoConfig.getInstance(getApplicationContext()));
		...
		// 设置加载失败显示图片
		AsyncImageLoader.getInstance(this).setLoadFailDrawable(
				R.drawable.empty_photo);
